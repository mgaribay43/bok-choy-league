...
async function shouldPollNow(): Promise<boolean> {
  ...
  const today = est.yyyymmdd;

  console.log(`[Poll Check] Checking NFL games for ${today} via ESPN API (EST)`);
  console.log(`[Poll Check] Current time EST: ${est.estString}`);
  ...
  try {
    ...
    for (const event of events) {
      ...
      const gameDate = new Date(new Date(competition.date).toLocaleString("en-US", { timeZone: "America/New_York" }));
      ...
      const timeDiff = nowEST.getTime() - gameDate.getTime();
...
export const pollWinProbabilities = onSchedule(
  ...
  async (_event: ScheduledEvent) => {
    const est = getESTDateObj();
    console.log(`[WinProb] Starting poll at EST: ${est.estString}`);
    ...
    try {
      ...
      for (let i = 0; i < matchupCount; i++) {
        ...
        // If a win probability of 1 has already been stored, skip further polling for this matchup
        const hasFinalWinProb =
          existingPoints.some(
            (pt: any) => pt.team1Pct === 1 || pt.team2Pct === 1
          );

        if (hasFinalWinProb) {
          console.log(`[WinProb] Skipping matchup ${matchupId} - already stored win probability of 1`);
          continue;
        }

        // If either team has 100% win probability, store this final point and stop polling for this matchup

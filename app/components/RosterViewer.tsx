...
export default function RosterPage() {
    ...
    const [week, setWeek] = useState<number>(1); // default week 1
    ...
    useEffect(() => {
        ...
        async function fetchRosterAndStats() {
            ...
            try {
                ...
                // Fetch player stats for selected week
                ...
                Object.values(playersArray).forEach((playerWrapper: any) => {
                    ...
                    // Extract stats (adjust based on your API response structure)
                    ...
                    statsMap[playerKey] = { byeWeek, fanPts, projPts };
                });
    ...
    // Normalize and order players (same as your logic)
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
            ...
            {loading && (
                <p className="text-center text-slate-500">Loading roster...</p>
            )}
                        ...
                        <div className="overflow-x-auto rounded-2xl shadow-xl border border-slate-200/50 bg-white">
                            <table className="min-w-full text-sm">
                                ...
                                <tbody>
                                    ...
                                    {orderedPlayers
                                        .filter(
                                            (p) => p.selectedPosition !== "BN" && p.selectedPosition !== "IR"
                                        )
                                        .map((player) => (
                                            <tr
                                            ...
                                            >
                                                ...
                                                <td className="p-3 border border-slate-200 text-center font-bold text-emerald-600">
                                                    {player.stats?.fanPts != null
                                                        ? player.stats.fanPts.toFixed(1)
                                                        : "-"}
                                                </td>
                                                <td className="p-3 border border-slate-200 text-center text-slate-700 font-medium">
                                                    {player.stats?.projPts != null
                                                        ? player.stats.projPts.toFixed(1)
                                                        : "-"}
                                                </td>
                                            </tr>
                                        ))}
